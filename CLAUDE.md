...
- **Overtake mode (`carPose`/`OT`)**: the car's position (local lane x + driver-seat z) has a single source of truth, `carPose()`, read by the camera, the exterior car, the minimap, and the stats via `cam.pose`. The `OT` timeline (seconds) drives one continuous easeIO speed profile from the follow position (zMin−d0) to established-ahead (zMax+14, then a slow continued pull-away) — no pause at the group's rear. The lateral drift to x = ROAD_W−1.1 is keyed to forward progress — not time — and spread wide (out across the whole approach run, completing level with the group's rear; back in over 9 m from zMax+4), so both sweeps are slow diagonals. The car never yaws — lane changes are pure translation. `state.overtake = {t0, d0}` is set by the ≫ button or the `ot` URL param; the whiteout covers the ahead→following reset, and a cancel freezes the pose (`overtake.freeze`) behind the same dip. The "looks like" stat blanks to – whenever the eye isn't behind the group.
