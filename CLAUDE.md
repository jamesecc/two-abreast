...
```
- URL params drive any state for screenshots: `view` (0=top-down … 1=driver), `chaos` 0–10, `n` 5–15, `pair` 0–100, `curb` 0.1–1.5, `car` 5–50, `flip=1` (right-hand traffic). Params also skip the auto-sweep.
...
- **Renderer**: hand-rolled perspective projection onto a 2D canvas (no Three.js — CSP). `camera()` returns a look-at basis; all drawing goes through near-plane-clipping helpers (`projPoly`/`fillPoly`/`seg`/`circle3d`/`sphere3d`) with painter's-algorithm ordering (ground → dashes → hedges → car → riders far-to-near → bonnet overlay). Heads use `sphere3d` (billboard) — planar circles render edge-on as lines from behind.
