...
- **Overtake mode (`carPose`/`OT`)**: the car's position (local lane x + driver-seat z) has a single source of truth, `carPose()`, read by the camera, the exterior car, the minimap, and the stats via `cam.pose`. The `OT` timeline (seconds) drives forward progress (easeIO from zMin−7 to zMax+14, then a slow continued pull-away); the lateral sweep to x = ROAD_W−1.1 is keyed to that forward progress — not to time — so the car crosses out and merges back on moving diagonals, never sliding sideways at zero relative speed. `state.overtake = {t0, d0}` is set by the ≫ button or the `ot` URL param; the whiteout covers the ahead→following reset, and a cancel freezes the pose (`overtake.freeze`) behind the same dip. The "looks like" stat blanks to – whenever the eye isn't behind the group.
